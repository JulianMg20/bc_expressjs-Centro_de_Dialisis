...
import { createPacienteSchema, updatePacienteSchema, idParamSchema } from "../validators/pacientes.schema.js";

export async function listar(req: Request, res: Response, next: NextFunction) {
  try { res.status(200).json({ data: await service.listar() }); }
  catch (err) { next(err); }
}
...
export async function crear(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.status(201).json({ data: await service.crear(data, req.user!.id) });
  } catch (err) { next(err); }
}
